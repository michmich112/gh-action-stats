...
export async function loginUserEntrypoint(data: LoginUserData, context: CallableContext): Promise<LoginUserDataReturn> {
  functions.logger.debug("[LoginUser][Start] - Starting execution on loginUser Function");
  if (!context.auth) {
    functions.logger.warn("[LoginUser][Unauthorized] - Function run is not authenticated with firebase.");
    return {
      code: 401,
  ...
  if (!data.GithubToken) {
    functions.logger.warn("[LoginUser][BadRequest] - Function run did not contain Github Token.");
    return {
      code: 400,
  ...
  try {
    ...
    functions.logger.debug("[LoginUser][Success] - Success in executing Login User");
    return {
      code: 200,
      message: "LoggedIn",
    ...
    functions.logger.error(`[LoginUser][ERROR]: error authenticating: ${(e as Error).message}`);
