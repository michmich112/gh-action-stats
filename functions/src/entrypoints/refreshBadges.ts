...
async function updateBadges(message: functions.pubsub.Message, context: functions.EventContext) {
  const { owner, repo } = message.json;
  if (owner === undefined || repo === undefined) {
    console.error(`[Refresh Badges][BadMessage] - attempted to refresh badges for owner ${owner} and repo ${repo}`);
    return;
  }
...
export const refreshBadges = functions.pubsub.topic("badges").onPublish(updateBadges);
